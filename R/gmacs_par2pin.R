#' Write .pin file
#'
#' Create GMACS .pin file in R from existing par file
#' @param par path to .par file.

#' @return File saved in specified location.
#' @examples gmacs_par2pin(input, file = "./AIGKC/models/2024/may/EAG/23.1b/gmacs.par")
#'
#' @export
#'
gmacs_par2pin <- function(par){
  ...
  if(version %in% c("2.20.31", "2.20.32", "2.20.33", "2.20.34", "2.20.34a")){
    ...
    out[last + 1,] <- "#series year season fleet sex maturity obs cv units timing rai_id"; last <- last + 1
    # index data frame
    ...
    out[last + 1,] <- "999 #--check on addition of RAI_id column"; last <- last + 1
