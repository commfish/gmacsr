...
if(is.null(version)){version = "2.20.39"}
...
if(version %in% c("2.20.21")){
    ...
    out$size_fit_summary %>%
      ...
      mutate(data = purrr::map(data, function(data) {
        ...
        sizes <- data %>% distinct(size) %>% pull(size) %>% .[-length(.)]

        for(j in 1:length(sizes)){
          for(i in 1:length(yrs)) {
            osa %>% add_row(year = yrs[i], size = sizes[j], osa_residual = res[j, i]) -> osa
          }
        }
        # join to data
        data <- left_join(data, osa, join_by(year, size))

      })) %>%
      unnest() -> out$size_fit_summary
...
if(version %in% c("2.20.31", "2.20.32", "2.20.33", "2.20.34", "2.20.34a", "2.20.34b")){
    # setup ----
    # Suppress the NA message in the coercion to double
    options(warn = -1)
    ...
    recruitment = as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    tagging = as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    growth = as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    penalties =  as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    priors = as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    initial_size = as.numeric(na.omit(as.numeric(allout[last + 1,]))); last <- last + 1
    ...
    rbind(catch, index, size, recruitment, tagging, growth) %>% as_tibble() %>%
      ...
      add_row(process = "priors", raw_lik = priors, net_lik = priors) %>%
      add_row(process = "initial_size", raw_lik = initial_size, net_lik = initial_size) %>%
      add_row(process = "positive_growth_penalty", raw_lik = positive_growth_penalty, net_lik = positive_growth_penalty) %>%
      add_row(process = "total", raw_lik = sum(.$raw_lik), net_lik = total) -> out$likelihoods_by_type
    ...
    ## growth
    tibble(raw_lik = as.numeric(na.omit(as.numeric(allout[last + 2,]))),
           pos_fun = as.numeric(na.omit(as.numeric(allout[last + 2,])))) %>%
      transmute(process = paste0("growth_", 1:nrow(.)), raw_lik, pos_fun) -> growth; last <- last + 5
    bind_rows(catch, index, size, rec_pen, tagging, growth) -> out$likelihoods_by_type_and_fleet
    ...
    bind_rows(list_tmp) %>%
      ...
      pivot_longer(2:(ncol(.)-1), names_to = "size", values_to = "n") %>%
      pivot_wider(names_from = type, values_from = n) %>%
      right_join(out$n_matrix, ., by = join_by(year, size)) %>%
      mutate(size = as.numeric(size)) -> out$n_matrix
    ...
    names(list_tmp) <- smats_names
    out$size_transition <- list_tmp
    last <- smats[2] + length(out$size_mid_points)

    # mature probability ----

    if(out$n_maturity > 1) {
      ## weight at size matrix
      tmp <- matrix(nrow = out$n_sex*length(out$mod_yrs), ncol = length(out$size_mid_points)+2)
      for(i in 1:nrow(tmp)) {
        tmp[i,] <- as.character(allout[last+4+i,1:ncol(tmp)])
      }
      as_tibble(tmp) %>%
        mutate_at(2:ncol(.), as.numeric) %>%
        rename_all(~c("sex", "year", out$size_mid_points)) %>%
        pivot_longer(3:ncol(.), names_to = "size", values_to = "prob_maturity") %>%
        mutate(size = as.numeric(size)) -> out$maturity_probability
    }
    last <- grep("#---", allout[,1])[12]
...
if(version %in% c("2.20.39")){
    ...
    ## check whether this section actually has data, or is just an empty
    ## header (happens when the model run didn't produce reference points,
    ## e.g. no OFL/BMSY calculated). When empty, the header row is
    ## immediately followed by ">EOD<" with no data rows in between.
    ref_end <- grep("#---", allout[,1])[2]
    first_ref_row <- str_flatten(allout[last + 3,], na.rm = TRUE)
    ref_data_present <- !grepl("^>EOD<", first_ref_row) && !is.na(first_ref_row) && first_ref_row != ""

    if (ref_data_present) {
      ## ref tibble
      tmp <- matrix(ncol = 3, nrow = length((last + 2):(ref_end - 5)))
      for(i in 1:nrow(tmp)) {
        tmp[i,] <- na.omit(as.numeric(allout[last + 2 + i,]))
      }
      as_tibble(tmp) %>%
      ...
      ## no reference points estimated for this run -- fill with empty/NA
      ## placeholders so downstream code referencing these fields doesn't error
      out$reference_points <- tibble(parameter_name = character(), estimate = numeric(),
                                     se = numeric(), est_quantity_count = numeric())
      out$mmb_curr   <- NA_real_
      out$ofl_tot    <- NA_real_
      out$bmsy       <- NA_real_
      out$b_bmsy     <- NA_real_
      out$f_msy_tot  <- NA_real_
      out$f_ofl_tot  <- NA_real_
      out$rbar_tot   <- NA_real_
    }
